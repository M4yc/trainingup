...
const ListWeek: React.FC = () => {
    ...
    return (
        <View style={styles.container}>
            {days.map(item =>(
                <View key={item.num} style={[styles.continerDay,{backgroundColor: item.trained ? Colors.primary : 'transparent'}]}>
                    <Text style={{color: '#fff'}}>{item.day}</Text>
                    <Text style={{color: '#fff'}}>{item.num.toString().padStart(2,'0')}</Text>
                </View>
            ))}
            </View>
    );
